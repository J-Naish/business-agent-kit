# Fulfillment, Pricing, Fees, and Offer Strategy

Use this reference to plan fulfillment method, pricing, fee risk, inventory, promotions, and Featured Offer competitiveness. Fees and program rules vary by marketplace and change often, so verify current Seller Central or Vendor Central terms before finalizing numbers.

## Fulfillment Options

| Method | Role | Strengths | Risks |
|---|---|---|---|
| Fulfillment by Amazon (FBA) | Amazon stores, picks, packs, ships, and handles customer service/returns | Prime eligibility, delivery trust, operational scale, often stronger Featured Offer competitiveness | Fulfillment fees, storage fees, long-term inventory risk, less direct control |
| Fulfilled by Merchant (seller-fulfilled) | Seller handles storage and shipping | More control, useful for bulky/fragile/custom products, can reduce some fees | Delivery promise, customer service, tracking, and account health burden |
| Seller Fulfilled Prime | Seller ships from its own operation while meeting Prime delivery standards where eligible | Prime badge without FBA; useful for strong logistics teams | Strict delivery, handling-time, volume, and performance requirements vary by marketplace |
| Hybrid | FBA for core demand and seller-fulfilled backup or special SKUs | Balances resilience and control | Requires inventory discipline and channel coordination |

## FBA Planning

Consider FBA when:

- Prime delivery trust is important in the category.
- The product is small or standard enough for fee economics to work.
- The seller can forecast demand and avoid stockouts or aging inventory.
- Customer service and returns complexity would otherwise constrain growth.

Watch:

- Fulfillment fee by size/weight tier.
- Monthly storage fees.
- Aged inventory and removal/disposal fees.
- Inbound Placement Service Fee or similar placement-related fees where applicable.
- Low-Inventory-Level Fee or similar days-of-supply fees where applicable.
- Storage Utilization Surcharge or other utilization-based storage fees where applicable.
- Packaging, prep, labeling, and inbound placement requirements.

Since 2024, several marketplaces, especially the U.S., have shifted more FBA cost pressure into inbound placement, low-inventory, aged-inventory, and storage-utilization fees. Treat these as first-class unit-economics inputs, not minor operational details.

## Pricing Strategy

Pricing affects conversion, organic rank, ad efficiency, and Featured Offer eligibility.

| Pricing question | Planning guidance |
|---|---|
| Is the price competitive? | Compare against top organic results, sponsored competitors, couponed prices, and shipping promise. |
| Does margin support paid traffic? | Calculate break-even ACoS before setting bids. |
| Is the product new? | Launch pricing or coupons can help overcome low review count, but should not train shoppers to wait for discounts. |
| Is the category deal-driven? | Align coupons, deals, and event promotions with inventory and margin. |
| Are variations priced logically? | Avoid confusing price jumps across size/color bundles. |

## Featured Offer Readiness

The Featured Offer is influenced by factors such as:

- Competitive price including shipping.
- Availability and inventory depth.
- Delivery speed and reliability.
- Seller account health.
- Fulfillment method.
- Customer service and return performance.

If a product repeatedly loses Featured Offer eligibility, diagnose offer quality before increasing ad spend.

Do not treat Featured Offer as a static single-winner state. Amazon can rotate eligible offers, and ad eligibility or conversion can change when price, delivery promise, inventory, or seller performance shifts.

## Profit and Break-Even ACoS

Use contribution margin to set the maximum sustainable ACoS.

```text
Contribution margin per unit =
Selling price
- referral fee
- fulfillment/shipping cost
- product cost
- promotion/discount
- variable operating cost

Break-even ACoS =
Contribution margin per unit / Selling price
```

Example:

```text
Selling price: 40.00
Variable costs and fees: 28.00
Contribution margin: 12.00
Break-even ACoS: 12.00 / 40.00 = 30%
```

Set target ACoS below break-even unless the launch plan intentionally trades margin for rank, reviews, or new customer acquisition.

## Inventory Planning

Advertising can create stockout risk. Before scaling:

- Estimate weeks of cover using recent sales plus expected ad-driven lift.
- Protect hero ASIN inventory during peak events.
- Avoid driving traffic to variants that will stock out.
- Reduce bids or budgets when inventory is constrained.
- Plan post-event inventory before promotional spikes.

## Promotion Planning

Common levers include coupons, deals, price discounts, bundles, Brand Tailored Promotions, and external traffic. Match promotion type to objective:

| Objective | Promotion posture |
|---|---|
| New product launch | Coupon or introductory price to reduce trial friction |
| Peak retail event | Deal plus increased budget on proven terms |
| Review count disadvantage | Temporary incentive through price, not review manipulation |
| Repeat purchase | Audience-based promotion where available |
| Inventory clearance | Discount plus controlled remarketing; protect brand perception |

Also consider Amazon-specific promotion and fulfillment tools where available:

- Subscribe & Save for replenishable categories.
- Prime Exclusive Discount for Prime member offers and major retail events.
- Lightning Deals, Best Deals, and event deals where eligibility and fees justify them.
- Brand Tailored Promotions for Brand Registry-gated audience segments.
- Buy with Prime or Multi-Channel Fulfillment when the brand also sends traffic to its own DTC site.
- Amazon Warehousing & Distribution or upstream storage products when bulk storage and FBA replenishment economics matter.

## Common Mistakes

- Treating media ACoS as profit without fees, product cost, discounts, and returns.
- Scaling ads into weak inventory coverage.
- Running competitor conquesting when the competing product has better price, rating, and delivery.
- Using FBA without monitoring aging inventory.
- Letting promotions hide an uncompetitive base price.
